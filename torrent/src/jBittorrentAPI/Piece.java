...
public class Piece {
    ...
    public Piece(int index, int length, int blockSize, byte[] sha1, TreeMap<Integer, Integer> m) {
        ...
        this.sha1 = sha1;
        this.filesAndoffset = Objects.requireNonNullElseGet(m, TreeMap::new);
    }
    ...
    public synchronized byte[] data(){
        ...
        for (Integer integer : this.pieceBlock.keySet()) data = Utils.concat(data, this.pieceBlock.get(integer));
        return data;
    }
    ...
    public synchronized boolean verify(){
        return Objects.requireNonNull(Utils.byteArrayToByteString(Utils.hash(this.data()))).matches(Utils.byteArrayToByteString(this.sha1));
    }
    ...
    public synchronized String toString(){
        ...
        s.append("Piece ").append(this.index).append("[").append(this.length).append("Bytes], part of file");
        if(this.filesAndoffset.size() > 1)
            s.append("s");
        for(Iterator it = this.filesAndoffset.keySet().iterator(); it.hasNext();){
            int key = (Integer) (it.next());
            s.append(" ").append(key).append(" [offset = ").append(this.filesAndoffset.get(key)).append("]");
            if(it.hasNext())
                s.append(" and");
        }
        return s.toString();
