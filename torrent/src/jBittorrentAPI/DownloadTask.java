...
public class DownloadTask extends Thread implements IncomingListener,
    ...
    public void run() {
        try {
            ...
            this.fireTaskCompleted(this.peer.toString(), UNKNOWN_HOST);
        } catch (IOException ioe) {
    ...
    public synchronized int checkDownloaded(){
        //this.downloaded = 0;
        return Long.valueOf(this.downloaded).intValue();
    }
    ...
    public synchronized void connectionClosed() {
        this.clear();
        this.fireTaskCompleted(this.peer.toString(), CONNECTION_REFUSED);
    }
    ...
    public synchronized void keepAliveSent() {
        if (System.currentTimeMillis() - this.lmrt > 180000) {
            this.clear();
            this.fireTaskCompleted(this.peer.toString(), TIMEOUT);
            return;
        }
