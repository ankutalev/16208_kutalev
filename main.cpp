...
EntriesList& EntriesList:: operator = (const EntriesList& t) {
    if (this!= &t) {
        Node* tmp = t.head;
        delete this; //???????????????????????????? нужно ли разделять деструктор и освобождение ресурсов если по сути для освобождения старых ресурсов нужно удалить старый объект
        while (tmp) {
            this->addElement(tmp->CurrentKey,tmp->Current);
            tmp=tmp->Next;
        }
...
bool EntriesList::Implace(const key& k , const value& v) {
...
HashTable&  HashTable :: operator = (const HashTable& t) {
    if (this!= &t) {
        ...
        // std::copy
        copy(t.data,t.data+t.capacity,data + capacity);
...
bool HashTable:: Implace (const Key& k , const Value& v) {
    size_t index = HashCalculation(k)% capacity;
    if (data[index].Implace(k,v))
        return true;
    return false;
}
int main()
